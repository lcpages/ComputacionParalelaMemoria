...
{ int i,j,d,im,dm;// dm es la distancia del código más parecido, im su índice
   ...
   double ave, ave_g;  
   int max,min, max_g, min_g , quo;  
	         
  #pragma omp parallel private(i,dm,d,im, min, max, ave, quo) 
  {	
      int size = 0; 
		quo =0;
	   max =  0, max_g = 0, ave_g = 0; 
	   min = MAX_LON + 1, min_g  = MAX_LON +1; 
	   ave = 0;
...
{ int err,err2,nt1,nt2,*ps,numeros;
  ...
  #pragma omp parallel sections
  {
	  ...
	  if ( err ) err = 2;
		...
		if ( ps == NULL ) {
		  ...
		  t = omp_get_wtime() - t; 
		  printf("Time needed to execute the function \"bucas_mas_parecidos\" is : %f seconds\n", t);
  ...
  printf("There were %d threads used to execute the program \n", nh);
